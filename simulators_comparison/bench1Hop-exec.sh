...
do
  ...
  java -jar IFogSim/iFogSim2_1hops_jar.jar ${i} ${valIFS} ${flowSize} ${delay} ${execCost}> outIFS.log 2>&1
  ...
  python main1_perso.py ${i} ${latVal} ${flowSize} ${delay} ${execCost}
  cd ${PREVID}
  ...
  ./simgrid/bench_1hop ./simgrid/platform_bench1hop.xml ${i} "${latVal}ms" ${flowSize} ${delay} ${execCost} > outSG.log 2>&1
  awk -f simgrid/parseSG.awk -v nbFlows=${i} latency=${latVal}ms flowSize=${flowSize} execCost=${execCost} outSG.log >> ${resFile}

  #ns3
  ./ns-3/wired_nhop --flowNumbers=${i} --latency=${latVal}ms --flowSize=${flowSize} --DelayBetweenFlows=${delay} --Seed=1 --nbHops=1 > outNS.log 2>&1
