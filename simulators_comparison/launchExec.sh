...
do
  ...
  do
    for k in ${nbExec};
    do
      ./simgrid/execScenario ./simgrid/platformWired_"${i}"core.xml ${k} ${j} 0 ${i} > outSG.log 2>&1
      awk -f simgrid/parseExec.awk -v nbFlows=${k} nbCore=${i} step=0 execSize=${j} outSG.log >> ${resFile}
      valIFS=` echo "${k}/1000.0" | bc -l`
      java -jar IFogSim/iFogSimExec.jar ${k} ${j} 0 ${i} > outIFS.log 2>&1
      awk -f IFogSim/parseExec.awk -v nbFlows=${k} nbCore=${i} step=0 execSize=${j} outIFS.log >> ${resFile}
    done
  done
done