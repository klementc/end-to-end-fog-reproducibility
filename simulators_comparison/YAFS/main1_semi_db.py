...
def create_json_topology(bw1, bw2, bw3, bw4, latsrc1, latsrc2, latdev, latdst):
    """
       TOPOLOGY DEFINITION
    ...
    sensor_2   = {"id": 4, "model": "sensor-device2", "IPT":1 * 10 ** 9, "RAM": 4000,"COST": 3,"WATT":40.0}
    actuator_dev2 = {"id": 3, "model": "midway-device", "IPT": 1 * 10 ** 9, "RAM": 4000,"COST": 3, "WATT": 40.0}
    actuator_dev3 = {"id": 5, "model": "midway-device2", "IPT": 1 * 10 ** 9, "RAM": 4000,"COST": 3, "WATT": 40.0}
    actuator_dev = {"id": 2, "model": "actuator-device", "IPT": 1 * 10 ** 9, "RAM": 4000,"COST": 3, "WATT": 40.0}

    link1 = {"s": 5, "d": 3, "BW": bw3/8, "PR": latdev/1000} # midway2 <-> midway
    link1bis = {"s": 3, "d": 1, "BW": bw1/8, "PR": latsrc1/1000} # midway <-> sensor
    link1ter = {"s": 3, "d": 4, "BW": bw2/8, "PR": latsrc2/1000} # midway <-> sensor2
    link1qad = {"s": 5, "d": 0, "BW": bw4/8, "PR": latdst/1000} # cloud <-> midway2
    link2 = {"s": 0, "d": 2, "BW": 50/8, "PR": 0.0} # actuator <-> cloud
    ...
    topology_json["entity"].append(actuator_dev3)
    topology_json["entity"].append(sensor_2)

    topology_json["link"].append(link1)
    topology_json["link"].append(link2)
    topology_json["link"].append(link1bis)
    topology_json["link"].append(link1ter)
    topology_json["link"].append(link1qad)
...
# @profile
def main(simulated_time, flowN, flowSize, delayBetweenFlows, execCost, bw1, bw2, bw3, bw4, latsrc1, latsrc2, latdev, latdst):
    ...
    """
    t = Topology()
    t_json = create_json_topology(bw1=bw1, bw2=bw2, bw3=bw3, bw4=bw4, latsrc1=latsrc1, latsrc2=latsrc2, latdev=latdev, latdst=latdst)
    t.load(t_json)
    ...
    for ic in range(flowN):
        ...
        pop.set_src_control({"model": "sensor-device2", "number":1,"message": app.get_message("M.A"), "distribution": dDistribution})
...
if __name__ == '__main__':
    ...
    flowN = 1
    latsrc0 = float(sys.argv[1])
    latsrc1 = float(sys.argv[2])
    latd12 = float(sys.argv[3])
    latdst = float(sys.argv[4])
    flowSize = int(sys.argv[5])
    delayBetweenFlows= 0
    execCost = int(sys.argv[6])
    bw1 = int(sys.argv[7])
    bw2 = int(sys.argv[8])
    bw3 = int(sys.argv[9])
    bw4 = int(sys.argv[10])
    ...
    main(simulated_time=200, flowN=flowN, flowSize=flowSize, delayBetweenFlows=delayBetweenFlows, execCost=execCost, bw1=bw1, bw2=bw2, bw3=bw3, bw4=bw4, latsrc1=latsrc0, latsrc2=latsrc1, latdev=latd12, latdst=latdst)
